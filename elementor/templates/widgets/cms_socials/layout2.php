<?php
	$widget->add_render_attribute( 'socials-wrap', 'class', 'row gutters-10 gutters-grid');
	...
	<div <?php etc_print_html($widget->get_render_attribute_string( 'socials-wrap' )); ?>>
		<?php foreach ($settings['social_list'] as $value): 
		    ...
		    //if(!isset($link_attrs['class'])){
		    	$link_attrs[] = 'class="'.$widget->get_setting('social_shape', 'circle').'"';
		    //}
			$icon_color       = $widget->get_setting('social_icon_color', 'white');
			$icon_color_hover = $widget->get_setting('social_icon_color_hover', 'white');
			$bg_color         = $widget->get_setting('social_bg_color', 'transparent');
			$bg_color_hover   = $widget->get_setting('social_bg_color_hover', 'secondary');
			$icon_size		  = !empty($widget->get_setting('social_size')['size']) ? $widget->get_setting('social_size')['size'] : '50';
