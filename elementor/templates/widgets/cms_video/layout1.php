...
$img_classes = '';
// wrap
$widget->add_render_attribute( 'wrap', 'class', 'cms-video-player relative d-inline-block relative pb-md-40 cms-video-play-btn-'.$widget->get_setting('video_play_btn_position', 'center-left'));
...
<div <?php etc_print_html($widget->get_render_attribute_string( 'wrap' )); ?>>
    <div class="relative">
        <?php 
            saniga_elementor_image_render($settings, [
                ...
                'default_img' => get_template_directory_uri() . '/elementor/templates/widgets/cms_video/layout-images/thumb-1.png'
            ]);
        ?>
        <?php if(!empty($video_link)) : ?>
            <div class="btn-video-wrap">
                <?php switch ($settings['play_icon_style']) {
                    case '1':
                    ?>
    ...
    <div class="cms-video-text pt-md-40 pr-md-40 bg-white ml-lg-n40">
        <div class="cms-video-title cms-heading text-20 lh-29 font-500i bg-<?php echo esc_attr($widget->get_setting('heading_bg','grey'));?> text-<?php echo esc_attr($widget->get_setting('heading_text_color', 'heading'));?> p-40"><?php echo esc_html($widget->get_setting('heading_text','Our Cleaning Experts Work To Deliver The Highest Quality Of Clean You Can Always Count.')); ?></div>
    </div>
</div>