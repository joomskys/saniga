...
$widget->add_render_attribute( 'wrap', 'class', 'bg-'.$widget->get_setting('bg_color', 'primary'));
//background image
$background_img = saniga_elementor_image_url_render($settings, [
    'id'            => 'background_img',
    'size'          => 'background_size',
    'default_thumb' => true,
    'default_img'   => get_template_directory_uri().'/elementor/templates/widgets/cms_fancy_box/layout-images/bg-11.jpg'
]);
...
<div <?php etc_print_html($widget->get_render_attribute_string( 'wrap' )); ?>>
    <div class="cms-saleoff absolute text-white bg-secondary circle text-30 font-700 text-center"><?php
        etc_print_html($widget->get_setting('saleoff11', ['size' => 50])['size'].'<sup>%</sup>');
    ?></div>
    <div class="row <?php echo saniga_elementor_align_class($settings);?>">
        ...
        <div class="col-12 cms-fancybox-content relative">
            <div <?php etc_print_html($widget->get_render_attribute_string( 'small-heading' )); ?>><?php
                etc_print_html($widget->get_setting('small_title','Hot & New'));
            ?></div>
            <div <?php etc_print_html($widget->get_render_attribute_string( 'heading' )); ?>><?php
                etc_print_html($widget->get_setting('title','Advanced Hand Sanitizer'));
            ?></div>
        </div>
        <?php 
