...
$widget->add_render_attribute( 'counter', [
    'class' => 'cms-counter-number cms-heading font-400 text-60 lh-60 text-'.$widget->get_setting('number_color','accent'),
...
$widget->add_render_attribute( 'title', [
    'class' => 'cms-counter-title cms-heading text-16 lh-26 pt-15 text-'.$widget->get_setting('title_color','')
] );
// Description 
...
<div <?php etc_print_html($widget->get_render_attribute_string( 'wrap' ));?>>
    ...
    <?php if ( $settings['description_text'] ) : ?>
        <div <?php etc_print_html($widget->get_render_attribute_string( 'description' )); ?>><?php etc_print_html($settings['description_text']); ?></div>
    <?php endif; ?>
    <div class="cms-counter-links pt-15 empty-none"><?php 
        foreach ($settings['counter_link'] as $value) {
            saniga_elementor_hyperlink_render($value,[
                'class'        => 'cms-counter-link font-700 text-'.$settings['link_color'].' text-hover-'.$settings['link_color_hover'],
