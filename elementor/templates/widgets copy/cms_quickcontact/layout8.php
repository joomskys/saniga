<?php 
// Dropdown Heading
$widget->add_render_attribute( 'dropdown_heading', 'class', 'dropdown-heading cms-transition text-'.$widget->get_setting('dropdown_heading_color','white'));
if($settings['dropdown_heading_color'] === 'custom' && !empty($settings['dropdown_heading_custom_color'])){
    $widget->add_render_attribute( 'dropdown_heading', 'style', 'color:'.$settings['dropdown_heading_custom_color']);
}
// Heading
...
$widget->add_render_attribute( 'background', 'class', 'cms-qc-dropdown-content cms-transition cms-radius-tl-8 overflow-hidden' );
// Gradient
...
?>
<div class="cms-qc-dropdown relative">
	<div <?php etc_print_html($widget->get_render_attribute_string( 'dropdown_heading' )); ?>><?php echo esc_html($widget->get_setting('dropdown_heading_text','24/7 Emergency')); ?>&nbsp;&nbsp;<span class="cmsi-angle-down"></span></div>
	<div <?php etc_print_html($widget->get_render_attribute_string( 'background' )); ?>>
		<div <?php etc_print_html($widget->get_render_attribute_string( 'gradient' )); ?>>
			<div class="p-40">
				<?php saniga_elementor_icon_render($settings,['class'=>'text-50 lh-50 mb-15 text-'.$widget->get_setting('heading_color','white'), 'default_icon' => [ 'value' => 'medical medical-medical', 'library' => 'cms-medical'] ]); ?>
		        <div <?php etc_print_html($widget->get_render_attribute_string( 'heading' )); ?>><?php 
		            echo esc_html($widget->get_setting('heading_text','Emergency Cases'));
		        ?></div>
	    		...
	    		<div class="cms-contact-button pt-23 text-24 cms-heading font-400">
	    			<a href="tel:<?php echo esc_attr($widget->get_setting('phone_number','01061245741'));?>" class="text-<?php echo esc_attr($widget->get_setting('phone_text_color','white'));?> text-hover-<?php echo esc_attr($widget->get_setting('phone_text_color_hover','white'));?>"><?php 
