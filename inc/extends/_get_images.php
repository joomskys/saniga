 ...
 */
function saniga_get_image_sizes() {
    global $_wp_additional_image_sizes;
    $sizes = array();
 ...
 *
 * @uses   saniga_get_image_sizes()
 * @param  string $size The image size for which to retrieve data.
 * @return bool|array $size Size data about an image size or false if the size doesn't exist.
 */
function saniga_get_image_size( $size ) {
    $sizes = saniga_get_image_sizes();
 ...
 *
 * @uses   saniga_get_image_size()
 * @param  string $size The image size for which to retrieve data.
 * @return bool|string $size Width of an image size or false if the size doesn't exist.
 */
function saniga_get_image_width( $size ) {
    if ( ! $size = saniga_get_image_size( $size ) ) {
        return false;
    }
    if ( isset( $size['width'] ) ) {
        return $size['width'];
 ...
 */
function saniga_get_image_height( $size ) {
    if ( ! $size = saniga_get_image_size( $size ) ) {
        return false;
    }
...
*/
if (!function_exists('saniga_default_image_thumbnail_url')) {
    function saniga_default_image_thumbnail_url($args = array())
    {
        ...
        extract($args);
        /* use saniga_resize_thumbnail( $attach_id = null, $img_url = null, $width, $height, $crop = false ) */
        global $_wp_additional_image_sizes;
        $image_sizes = saniga_get_image_sizes();
        $size = explode('x', $size);
        $size_use = $size[0];
...
*/
if (!function_exists('saniga_default_image_thumbnail')) {
    function saniga_default_image_thumbnail($args = array())
    {
        $args = wp_parse_args($args, array(
            'size'        => 'large',
            'default_img' => '/wp-content/themes/' . get_template() . '/assets/images/placeholder/no-image.jpg',
...
* 
*/
if ( ! function_exists( 'saniga_resize_thumbnail' ) ) {
    /**
     * @param int $attach_id
     ...
     * @return array
     */
    function saniga_resize_thumbnail( $attach_id = null, $img_url = null, $width, $height, $crop = false ) {
...
if(!function_exists('saniga_image_by_size')){
    function saniga_image_by_size( $args = []) {
        ...
        if(empty($id) ){
            ...
            $saniga_image_by_size =  wp_get_attachment_image( $id, $size, '', array('class' => trim($class)) );
        } else {
            if ( is_string( $size ) ) {
            ...
            if ( is_array( $size ) ) {
                ...
                $p_img = saniga_resize_thumbnail( $id, null, $size[0], $size[1], true );
                ...
                $attributes = saniga_stringify_attributes( $img_atts );
                if($show_image){
                    $saniga_image_by_size = '<img ' . $attributes . ' />';
                } else {
                    $saniga_image_by_size = '<div style="width:'.$p_img['width'].'px;height:'.$p_img['height'].'px;max-width:100%;"></div>';
                }
            }
        }

        if($echo)
            echo saniga_html($args['before'].$saniga_image_by_size.$args['after']);
        else 
            return $args['before'].$saniga_image_by_size.$args['after'];
    }
}
 ...
 * @return string
 */
function saniga_stringify_attributes( $attributes ) {
    $atts = array();
 ...
 *
*/
if(!function_exists('saniga_image_dimensions')){
    function saniga_image_dimensions( $id = null , $size = 'medium', $dimensions = 'height', $echo = false ) {
        global $_wp_additional_image_sizes;
